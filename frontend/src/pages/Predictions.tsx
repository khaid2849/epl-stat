...
export default function Predictions() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-white mb-6">FPL Predictions</h1>

      {/* Filters */}
      <div className="bg-navy-light border border-navy-border rounded-lg p-4 mb-5">
        <div className="flex gap-2 flex-wrap mb-3">
          {positions.map(p => (
            <button
              key={p}
              onClick={() => setPos(p)}
              className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${pos === p ? 'text-black' : 'bg-navy border border-navy-border text-gray-400 hover:text-white'}`}
              style={pos === p ? { backgroundColor: p === 'All' ? '#FFD700' : POSITION_COLORS[p] } : {}}
            >
              {p}
            </button>
          ))}
      ...
      {errP && (
        <div className="text-red-400 text-center py-6">
          Failed to load predictions. Train ML models first: <code className="text-gray-300">python -m backend.ml.train</code>
        </div>
      )}
      {!loadP && !errP && predictions.length === 0 && (
        <div className="text-center py-12 text-gray-400">
          ...
          <div className="text-sm">Run <code className="text-gray-300">python -m backend.ml.train</code> to generate predictions</div>
        </div>
      )}
      ...
      {/* Best XI */}
      <div className="mt-8">
        <h2 className="text-xl font-bold text-white mb-4">Best XI</h2>
        {loadXI && <Skeleton className="h-80" />}
        {!loadXI && !bestXI && (
          <div className="text-center py-8 text-gray-400 border border-navy-border rounded-lg">
        ...
        <div className="mt-8">
          ...
          <div className="flex gap-2 mb-4">
            {(['GKP', 'DEF', 'MID', 'FWD'] as const).map(p => (
              <button
                key={p}
                onClick={() => setTopTab(p)}
                className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${topTab === p ? 'text-black' : 'bg-navy-light border border-navy-border text-gray-400 hover:text-white'}`}
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {(topPicks[topTab] ?? []).map((pick, i) => (
              <div key={pick.player_id} className="bg-navy-light border border-navy-border rounded-xl p-4 flex gap-3 items-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-gray-400">{pick.team_short} · {formatCost(pick.now_cost)}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-gold font-bold">{formatNum(pick.predicted_points, 1)} pts</span>
