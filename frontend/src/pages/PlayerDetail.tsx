...
export default function PlayerDetail() {
  ...
  if (error || !player) return (
    <div className="max-w-4xl mx-auto px-4 py-12 text-center">
      ...
      <Link to="/players" className="text-gold hover:underline mt-2 inline-block">← Back to Players</Link>
    </div>
  )
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <Link to="/players" className="text-gray-400 hover:text-white text-sm mb-4 inline-block">← Back to Players</Link>

      {/* Header */}
      <div className="bg-navy-light border border-navy-border rounded-xl p-5 mb-5 flex items-start gap-5 flex-wrap">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <Link to={`/teams/${encodeURIComponent(player.team_short)}`} className="text-gray-300 hover:text-white">{player.team_short}</Link>
          </div>
          <div className="flex gap-4 flex-wrap text-sm">
            <div><span className="text-gray-400">Price: </span><span className="text-white font-semibold">{formatCost(player.now_cost)}</span></div>
            <div><span className="text-gray-400">Ownership: </span><span className="text-white font-semibold">{formatNum(player.selected_by_percent, 1)}%</span></div>
            <div><span className="text-gray-400">Form: </span><span className="text-white font-semibold">{formatNum(player.form, 1)}</span></div>
            <div><span className="text-gray-400">PPG: </span><span className="text-white font-semibold">{formatNum(player.points_per_game, 1)}</span></div>
          </div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-5">
        {statCards.map(s => (
          <div key={s.label} className="bg-navy-light border border-navy-border rounded-lg p-3 text-center">
            ...
            <div className={`text-xl font-bold ${s.highlight ? 'text-gold' : 'text-white'}`}>{s.value ?? 0}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-5">
        {/* GW History Chart */}
        <div className="bg-navy-light border border-navy-border rounded-xl p-4">
          <h2 className="text-sm font-semibold text-gray-400 mb-3">Gameweek Points (Last 10)</h2>
          {gwHistory.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={gwHistory} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#21262D" />
                <XAxis dataKey="gameweek" tick={{ fill: '#8B949E', fontSize: 10 }} />
            ...
            <div className="text-gray-500 text-sm text-center py-12">No gameweek history available</div>
          )}
        </div>

        {/* Radar Chart */}
        <div className="bg-navy-light border border-navy-border rounded-xl p-4">
          <h2 className="text-sm font-semibold text-gray-400 mb-3">Player Profile</h2>
      ...
      {prediction && (
        <div className="bg-navy-light border border-navy-border rounded-xl p-5">
          ...
          <div className="flex items-center gap-4 mb-4 flex-wrap">
            <div>
              <div className="text-gray-400 text-sm">Predicted Points</div>
              <div className="text-3xl font-bold text-gold">{formatNum(prediction.predicted_points, 1)}</div>
            </div>
            <div>
